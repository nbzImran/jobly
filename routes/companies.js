...
router.get("/", async function (req, res, next) {
  try {
    const { name, minEmployees, maxEmployees, ...extraFilters } = req.query;


      // Ensure no unsupported filters are used
      if (Object.keys(extraFilters).length > 0) {
        throw new BadRequestError(`Invalid filter(s): ${Object.keys(extraFilters).join(", ")}`);
      }

    // Ensure minEmployees and maxEmployees are valid numbers
